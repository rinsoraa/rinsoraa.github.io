...
- 视觉中心**明显偏左**：扇形包络（含最外那一圈）的右边界在 1440×900 与 1152×720 下
  都落在 **42% 左右**，右边 58% 留给常驻的档案轨。
  ⚠️ 这个数字**只有一个落点**：JS 的 `RAIL_X = 0.44`（CSS 的 `var(--mm-rail-x,44%)`
  是同一数字的 fallback，两处必须相等，回归装置 O29 会拦）。
  它同时决定「右侧轨道多宽」与「左侧唱片簇被推到多左」
  （`apexX = railX - gap - selR`，见下面的构造性约束①）。
  所以「唱片再往左一点」和「档案再宽一点」是**同一个改动的两面**，不要拆成两处调参。
...
| 当前在播的歌不在窗口里 | 进厅时自动把扇形转到它，并展开它的档案（**重进也成立** —— 见下面第 4 条不变量） |
