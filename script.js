...
(() => {
  ...
  /* ============================================================
     展厅自己的「退出展厅」按钮 —— 也交给 exitMuseum()
     ------------------------------------------------------------
     ⚠️⚠️ 为什么必须接这一根线（用户反馈 4）：
       退出展厅这件事是**两套状态**一起关：本文件管「小窝淡出」
       （body.museum-open 的模糊/降透明度）与 URL 里的 #museum，
       music-museum.js 只管它自己那层场景 + 加载层。
       按钮原来在 music-museum.js 里直接调 exit()，只关了后一半 ——
       于是人退出来了，页面却还糊着（而且 #museum 还在 URL 上，
       刷新会又弹回展厅），要再按一次 Esc 才被这里的 exitMuseum 收拾干净。
